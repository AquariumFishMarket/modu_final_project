// Mock 상품 데이터
import { Product } from "../types/product";

export const mockProducts: Product[] = [
  {
    id: "1",
    itemName: "베타 피쉬 하프문 블루 레드 페어",
    price: 45000,
    itemImage: ["/img/fish/betta-1.jpg", "/img/fish/betta-2.jpg"],
    link: "/product/1",
    description: `아름다운 하프문 베타 피쉬 페어입니다. 🐠
    
직접 번식시킨 건강한 베타로, 색깔이 정말 선명하고 아름답습니다.
수컷은 진한 블루에 레드 포인트가 있고, 암컷은 연한 블루 톤입니다.

특징:
- 크기: 수컷 6cm, 암컷 4.5cm
- 나이: 4개월령
- 건강상태: 매우 양호
- 번식 가능한 성체

베타 전용 사료와 컨디셔닝 가이드를 함께 드립니다.
베타 키우기가 처음이신분들께도 자세한 사육 가이드 제공해드려요!`,
    createdAt: "2024-11-04T08:30:00Z",
    interactions: {
      views: 125,
      likes: 34,
      chatCount: 8,
    },
  },
  {
    id: "2",
    itemName: "구피 트리오 (레드 모자이크, 블루 그라스, 옐로우)",
    price: 18000,
    itemImage: [
      "/img/fish/guppy-red.jpg",
      "/img/fish/guppy-blue.jpg",
      "/img/fish/guppy-yellow.jpg",
    ],
    link: "/product/2",
    description: `색상이 다양하고 아름다운 구피 트리오입니다! 🌈
    
번식력이 뛰어나고 키우기 쉬워서 초보자분들께 강력 추천합니다.
이미 임신한 암컷들도 포함되어 있어서 곧 치어들을 보실 수 있을 거예요!

포함 구성:
- 레드 모자이크 구피 (수컷 1, 암컷 2)
- 블루 그라스 구피 (수컷 1, 암컷 1) 
- 옐로우 구피 (수컷 1, 암컷 1)
- 총 6마리

모두 건강하고 활발하며, 먹이도 잘 먹습니다.
치어 사육에 대한 팁도 함께 알려드려요!`,
    createdAt: "2024-11-04T06:15:00Z", // 4시간 전
    interactions: {
      views: 89,
      likes: 23,
      chatCount: 5,
    },
  },
  {
    id: "3",
    itemName: "네온테트라 스쿨링 세트 (20마리)",
    price: 32000,
    itemImage: "/img/fish/neon-tetra-school.jpg",
    link: "/product/3",
    description: `아름다운 스쿨링을 보여주는 네온테트라 20마리 세트입니다! ✨
    
모두 같은 사이즈로 통일되어 있어서 스쿨링이 더욱 아름답게 보입니다.
초보자도 키우기 쉽고, 어항이 화려하고 활기차게 변해요!

특징:
- 크기: 모두 2.5-3cm로 균일
- 건강한 개체들만 선별
- 이미 우리 어항에 적응된 상태
- 생존율 높음

평화로운 성격이라 다른 소형 열대어들과 함께 키우기 좋습니다.`,
    createdAt: "2024-11-03T20:45:00Z", // 1일 전
    interactions: {
      views: 67,
      likes: 19,
      chatCount: 3,
    },
  },
  {
    id: "4",
    itemName: "아쿠아스케이핑용 모스볼 세트 (10개입)",
    price: 25000,
    itemImage: "/img/plants/moss-ball-set.jpg",
    link: "/product/4",
    description: `아쿠아스케이핑에 완벽한 자연산 모스볼 세트입니다! 🌿
    
직접 채취한 신선한 모스로 만든 볼들로, 어항의 자연스러운 분위기를 연출해줍니다.
물고기들의 숨을 곳도 제공하고 수질 정화에도 도움이 됩니다.

구성품:
- 대형 모스볼 4개 (지름 5-7cm)
- 중형 모스볼 6개 (지름 3-5cm)
- 관리 가이드 포함`,
    createdAt: "2024-11-02T14:20:00Z", // 2일 전
    interactions: {
      views: 45,
      likes: 12,
      chatCount: 2,
    },
  },
  {
    id: "5",
    itemName: "안시스트루스 알비노 브리딩 페어",
    price: 35000,
    itemImage: "/img/fish/ancistrus-1.png",

    link: "/product/5",
    description: `청소부 물고기로 유명한 안시스트루스 알비노 브리딩 페어입니다! 🧹
    
어항 청소도 해주고 번식도 가능한 일석이조 물고기예요.
이미 한 번 산란 경험이 있는 건강한 성체들입니다.

특징:
- 크기: 수컷 12cm, 암컷 10cm
- 매우 활발하게 이끼와 잔여사료 청소
- 온순한 성격으로 다른 물고기와 잘 어울림
- 동굴이나 나무 장식을 좋아함`,
    createdAt: "2024-11-01T16:30:00Z", // 3일 전
    interactions: {
      views: 156,
      likes: 41,
      chatCount: 12,
    },
  },
];

// 단일 상품 조회
export const getProductById = (id: string): Product | undefined => {
  return mockProducts.find((product) => product.id === id);
};

// 가격대별 상품 조회
export const getProductsByPriceRange = (
  minPrice: number,
  maxPrice: number
): Product[] => {
  return mockProducts.filter(
    (product) => product.price >= minPrice && product.price <= maxPrice
  );
};

// 검색 기능
export const searchProducts = (query: string): Product[] => {
  const lowercaseQuery = query.toLowerCase();
  return mockProducts.filter(
    (product) =>
      product.itemName.toLowerCase().includes(lowercaseQuery) ||
      product.description.toLowerCase().includes(lowercaseQuery)
  );
};
